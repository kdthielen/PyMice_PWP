...
def H_fr(T_f,T_so,u_star_ocean):      #heat content of mixedlayer - potential to go to ice
    return rho_ocean_ref*cp_ocean*u_star_ocean*(T_f-T_so)


def Concentration_roc(A,T_so,T_f,h_ice,u_star_ocean,rho_ml,A_max,R_b):
    if((A<A_max)&(T_so<T_f)):
        phi_A=H_fr(T_f,T_so,u_star_ocean)*(1.-A)/(Latent_fusion*rho_ice_ref*h_ice)
    elif((A>0) & (T_so>T_f)):
        phi_A=H_fr(T_f,T_so,rho_ml)*(1.-R_b)*(1.-A)/(Latent_fusion*rho_ice_ref*h_ice)
    else:
        phi_A=0
    return phi_A

def Concentration_step(A,phi_A,dt):
    A=A+phi_A*dt
    return A

def basal_heat_flux(A,T_so,T_f,u_star_ocean,R_b):
    if((A>0)& (T_so>T_f)):
        F_sb=-H_fr(T_f,T_so,u_star_ocean)*R_b*(1-A)
    else:
        F_sb=0
    return F_sb

def F_MI(T_ml,T_f,u_star_ice):
    return rho_ocean_ref*cp_ocean*Stanton*u_star_ice*(T_ml-T_f)

def ice_thickness_roc(cond,mi,basal):
    return (cond-mi-basal)/(rho_ice_ref*Latent_fusion)

def ice_thickness_step(h_i,phi_h,h_ice_min,dt):
    h=h_i+phi_h*dt
    if h<h_ice_min:
        h=h_ice_min
    return h